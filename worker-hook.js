...
// -----------------------------------------------------------------------------
// Twitch のライブ広告（SSAI ミッドロール／プリロール）を除去します。
//
// 実装:  pixeltris/TwitchAdSolutions の "video-swap-new" (v1.55) をそのまま採用。
//        - Worker をラップして HLS プレイリスト取得(fetch)をフック
//        - 広告セグメント(stitched-ad)を検出したら別プレイヤータイプの
//          広告なしストリームへ差し替え／広告セグメントを除去(strip)
//        - window 側で gql の認証ヘッダを捕捉し worker へ渡す（バックアップ取得に必須）
//        - タブ非表示時に Twitch が再生を止めるのを防止
//
// 出典: https://github.com/pixeltris/TwitchAdSolutions (video-swap-new)
//       userscript ヘッダのみ除去し、拡張機能の MAIN world スクリプトとして読み込む。
//       ※ユーザーの明示的な承認のもと採用。
// -----------------------------------------------------------------------------
(function() {
    'use strict';
    ...
    function updateAdblockBanner(data) {
        // 左上の「Blocking ads」表示は出さない方針。
        // 既存のオーバーレイが残っていれば取り除くだけにして、新規には作らない。
        const existing = document.querySelector('.video-player .adblock-overlay');
        if (existing != null) {
            existing.remove();
        }
    }
